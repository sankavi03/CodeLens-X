    ...
    <div className="min-h-screen w-screen flex flex-col md:flex-row bg-[#0D1117] text-[#F0F6FC] font-sans antialiased relative overflow-hidden">
      ...
      {success && (
        <div className="fixed inset-0 bg-[#0D1117] z-50 flex flex-col items-center justify-center font-mono">
          ...
          <span className="text-xs text-[#8B949E] mt-1.5 animate-pulse">Decrypting workspace console...</span>
        </div>
      )}

      {/* LEFT COLUMN: Brand Identity (55%) */}
      <div className="w-full md:w-[55%] border-b md:border-b-0 md:border-r border-[#30363D] bg-[#161B22]/30 p-8 md:p-16 flex flex-col justify-between select-none">
        ...
        <div className="flex items-center gap-3 border-t border-[#30363D]/40 pt-6 mt-8 md:mt-0 max-w-sm">
          ...
          <div className="text-[9px] font-mono text-[#8B949E] leading-relaxed text-left">
            "Hi, I'm <span className="text-white font-bold">Lens</span>. I'll help you dissect imports and mapping patterns once we are initialized."
          </div>
        </div>
      </div>

      {/* RIGHT COLUMN: Centered Login Card (45%) */}
      <div className="flex-1 flex items-center justify-center p-8 md:p-16 relative">
        <div className="w-full max-w-[440px] bg-[#161B22] border border-[#30363D] p-8 rounded-xl shadow-xl flex flex-col justify-center">
          
          <div className="text-left mb-6 font-mono select-none">
            <h2 className="text-lg font-bold text-white mb-1">Welcome back.</h2>
            <p className="text-[10px] text-[#8B949E]">Continue exploring your workspace.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="text-left font-mono">
              <label className="block text-[10px] font-medium text-[#8B949E] uppercase mb-1.5">Username or Email</label>
              <div className="relative">
                <input
                  ...
                  placeholder="Choose unique username or email"
            ...
            <div className="flex items-center justify-between text-[10px] font-mono text-[#8B949E] select-none pt-1">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  className="rounded border-[#30363D] bg-[#0D1117] accent-[#7C5CFC]"
                />
                <span>Remember Me</span>
              </label>
              <button 
                type="button" 
                onClick={() => addToast('Reset instructions sent if email exists.', 'info')} 
          ...
          <div className="text-center mt-6 text-[10px] text-[#8B949E] font-mono select-none">
            New developer?{' '}
            <Link to="/register" className="text-[#7C5CFC] hover:underline font-bold">
